...
const vcard = 'BEGIN:VCARD\n' +
    ...
    'FN:teuz\n' +
    'ORG:Dono do BOT;\n' +
    'TEL;type=CELL;type=VOICE;waid=553197362337:+55 (31) 9736-2337\n' +
    'END:VCARD'
...
//  tiringa.logger.level = 'trace'
    ...
    tiringa.on('qr', () => {
        console.log(color('👆'), color(' Escanear o código acima para iniciar o Israel-BOT!'))
    })
    fs.existsSync('./tiringa.json') && tiringa.loadAuthInfo('./tiringa.json')
    tiringa.on('connecting', () => {
        start('2', 'Conectando o Israel-BOT...')
    })
    tiringa.on('open', () => {
            success('2', 'Israel-BOT conectado!!!')
        })
  ...
  tiringa.logger.error('Reconectando...')
    ...
    tiringa.on('chat-update', async(mek) => {
        try {
            ...
            const ownerNumber = [`553197362337@s.whatsapp.net`, `553184103362@s.whatsapp.net`]
            const liaN = [`0@s.whatsapp.net`]
            const ownerNumberB = [`553197362337@s.whatsapp.net`]
              ...
              //   await dappauhuy(dapuhy, MessageType.text, tescuk, `${ucapanFakereply}`)
               ...
               case 'x9':
                ...
                if (mentioned.length !== 0){
                    ...
                    if (quotedMsg.sender.match('553197362337')) return reply(`🤨`)
...
Link: ${i.swipeUpLink}\n\n𝗜𝗚 𝗦𝗧𝗢𝗥𝗬\n`
                 ...
                 case 'baiano':
             const baiano = `553197362337@s.whatsapp.net`
                    const tame = {
                        text: `@${baiano.split("@")[0]} `,
                        contextInfo: {
                            mentionedJid: [baiano]
...
e upar seu level
  ═════◄••LEVEL••►═════`*/
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        await ffmpeg(`./${media}`)
                            ...
                            .on('end', function() {
                                console.log('Finish')
                                exec(`webpmux -set exif ${addMetadata('Israel-BOT', 'TeuzPabliN')} ${ran} -o ${ran}`, async(error) => {
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        exec(`ffmpeg -i ${media} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 800:800 ${rano}`, (err) => {
                            fs.unlinkSync(media)
                            exec(`webpmux -set exif ${addMetadata('Israel-BOT', 'TeuzPabliN')} ${rano} -o ${rano}`, async(error) => {
                ...
                case 'bugreport':
                    ...
                    tiringa.sendMessage('553197362337@s.whatsapp.net', options, text, {
                        quoted: mek
                    })
...
`
  ...
  *◍₊ิ۪͡୭⇾Total de membros: ${participants.length} membros`
                ...
                case 'plaquinha':
					...
					teks = body.slice(6)
					if (teks.length > 6) return reply('O texto é longo, até 6 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`https://tiovoyan.sirv.com/Api/Screenshot_2021-04-08-23-34-04-1.png?text.0.=${teks}&text.0.position.x=-35%25&text.0.position.y=-11%25&text.0.size=50&text.0.color=000000&text.0.opacity=50&text.0.font.family=Homemade%20Apple&text.0.outline.blur=100&text.0.outline.opacity=88" width="458" height="661" alt="" />`)
                ...
                case 'plaquinha2':
					...
					if (teks.length > 6) return reply('O texto é longo, até 6 caracteres')
					reply('*Estou fazendo, se der erro tente novamente ✓*')
					buffer = await getBuffer(`<https://tiovoyan.sirv.com/Api/52c5c33f0f4525681184055c2f023e59.jpg?text.0.text=Nome&text.0.position.x=-2%25&text.0.position.y=-70%25&text.0.size=45&text.0.color=000000&text.0.font.family=Homemade%20Apple&text.0.background.opacity=33&text.0.outline.blur=100&text.0.outline.opacity=88" width="700" height="700" alt="" />`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '✓'})
				break
...
você se registrou, digite ${prefix}menu para listar meus comandos`
            ...
            //   case 'facebookpage':
              ...
              case 'banir':
					...
					if (!isGroupAdmins) return reply(vsfd imundo tu não é adm)
                ...
                case "ppt":
                    ...
                    if (vit == "derrota") {
                        var tes = "A vitória é do Israel-BOT"
                    }
                    if (vit == "empate") {
                        var tes = "O jogo terminou em empate"
                    }
                    reply(`Israel-BOT jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
                ...
                default:
                    ...
                    if (isCmd) {
                        if (!mek.key.fromMe) {
                           ...
                           const baino = `553197362337@s.whatsapp.net`
                          const tamme = {
